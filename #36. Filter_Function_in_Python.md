...
## 4. How `filter()` Works Internally

1. Takes one element from the iterable
2. Passes it to the given function
3. If the function returns `True`, the element is included
4. If the function returns `False`, the element is excluded
5. Repeats this process for all elements

This process removes unwanted data without modifying the original collection.

---


## 5. Using `filter()` with a Custom Function
