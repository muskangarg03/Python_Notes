...
* When Python encounters `break`, it **exits the entire loop** instantly.
* The loop body after `break` does not execute.
...
* When Python sees `continue`, it **jumps directly** to the next loop cycle.
* The remaining statements in that iteration are ignored.
...
Adding `pass` avoids syntax errors.
